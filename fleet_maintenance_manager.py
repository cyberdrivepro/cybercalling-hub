...
class FleetMaintenanceManager:
    ...
    def _load_vault(self) -> Dict[str, Any]:
        """Loads maintenance vault from SQLite backup or disk JSON."""
        # 1. Check SQLite backup first for persistent cross-restart state
        try:
            import sqlite3
            db_path = os.path.join(DATA_DIR, "cybercalling.db")
            if os.path.exists(db_path):
                conn = sqlite3.connect(db_path, timeout=5)
                c = conn.cursor()
                c.execute("""
                    CREATE TABLE IF NOT EXISTS fleet_maintenance_store (
                        id INTEGER PRIMARY KEY,
                        state_json TEXT,
                        updated_at REAL
                    )
                """)
                c.execute("SELECT state_json FROM fleet_maintenance_store WHERE id = 1")
                row = c.fetchone()
                conn.close()
                if row and row[0]:
                    sql_data = json.loads(row[0])
                    if sql_data.get("global_maintenance") or any(b.get("in_maintenance") for b in sql_data.get("bots", {}).values()):
                        return sql_data
        except Exception:
            pass
    ...
    def _save_vault(self):
        """Atomically persists maintenance state to JSON and SQLite database."""
        try:
        ...
        try:
            ...
            """)
            c.execute("INSERT OR REPLACE INTO fleet_maintenance_store (id, state_json, updated_at) VALUES (1, ?, ?)",
    ...
    def is_active(self, bot_key: str = "caller_bot") -> bool:
        """Returns True if global maintenance is active or this specific bot is locked."""
        now = time.time()
        if self.state.get("global_maintenance"):
            g_end = self.state.get("global_end_time", 0)
            if g_end == 0 or now < g_end:
                return True
        b_data = self.state.get("bots", {}).get(bot_key, {})
        if b_data.get("in_maintenance"):
            b_end = b_data.get("end_time", 0)
            if b_end == 0 or now < b_end:
                return True
        return False

    def get_admin_maint_banner(self, bot_key: str = "caller_bot") -> Optional[str]:
        """
        Renders an informative high-visibility header banner for the Admin,
        clearly displaying maintenance state, countdown / indefinite lock, and reason.
        """
        if not self.is_active(bot_key):
            return None

        now = time.time()
        g_on = self.state.get("global_maintenance", False)
        b_data = self.state.get("bots", {}).get(bot_key, {})
        end_t = self.state.get("global_end_time", 0) if g_on else b_data.get("end_time", 0)
        start_t = self.state.get("global_start_time", 0) if g_on else b_data.get("start_time", 0)
        reason = self.state.get("global_reason") if g_on else b_data.get("reason", "Scheduled System Maintenance")

        if end_t > now:
            rem_sec = int(end_t - now)
            rem_m = rem_sec // 60
            rem_s = rem_sec % 60
            tot_sec = max(1.0, end_t - start_t) if (start_t and end_t > start_t) else max(1.0, rem_sec)
            elapsed = max(0.0, now - start_t) if start_t else 0
            pct = min(100.0, (elapsed / tot_sec) * 100.0)
            bar = generate_progress_bar(pct, total_blocks=10)
            time_str = f"`{rem_m}m {rem_s}s remaining` `[{bar}]` *{pct:.0f}%*"
        else:
            time_str = "`🔒 Indefinite Lock (No auto-unlock)`"

        scope_str = "🌐 Global Fleet Maintenance (All 6 Bots)" if g_on else f"🛠️ Bot Node: `{bot_key}`"

        return (
            f"🛠️ *[FLEET MAINTENANCE MODE: ACTIVE 🔴]*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"• 🔒 *Scope:* {scope_str}\n"
            f"• ⏳ *Time Window:* {time_str}\n"
            f"• 📝 *Notice:* _{reason}_\n"
            f"• 👥 *Public Users:* **100% BLOCKED** (Showing Live Countdown Card)\n"
            f"• 👑 *Admin Access:* **ACTIVE** (Developer Bypass Enabled)\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
